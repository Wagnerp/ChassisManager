...
namespace Microsoft.GFS.WCS.WcsCli
{
    ...
    internal static class WcsCliConstants
    {
        ...
        //public const string runWinCommand = "runwincmd";
        #endregion
        ...
        public const string getinfoBatteriesHeader = "== Batteries ==";
        ...
        public const uint readsclogNumberEntries = 100;  // Number of blade log entries to read
...
wcscli -getchassisinfo                 Get information about
                                       blades, power supplies, batteries and Chassis Manager.

wcscli -getchassishealth               Get health status for blades, power supplies, batteries and Fan

wcscli -getbladeinfo                   Get information about blade.

wcscli -getbladehealth                 Get health information for blade, includes CPU info,
                                       Memory info, Disk info (JBOD only), PCIE info, Sensor info and Fru Info.
                                       This information can be requested separately using 
                                       command options.

wcscli -updatepsufw                    Update PSU firmware 

wcscli -getpsufwstatus                 Get PSU firmware revision and update status.
...
wcscli -getbladedefaultpowerstate      Get the default power on state of a blade.

wcscli -setbladeactivepowercycle       Power cycle blade.

wcscli -getnextboot                    This command gets the pending boot order to be applied 
                                       the next time the blade boots. 
                                       Once the boot order is applied, GetNextBoot will return 
                                       the default value of NoOverRide.
...
wcscli -setdatasafepoweron             Datasafe Power ON (Active power state) blade. 

wcscli -setbladedatasafeoff            Datasafe blade soft Power OFF 

wcscli -setdatasafepoweroff            Datasafe Power OFF (Active power state) blade. 
...
wcscli -setbladepsualertdpc            Sets psu alert default power cap for the blade

wcscli -getbladepsualertdpc            Gets psu alert default power cap for the blade
...
wcscli -getchassismanagerassetinfo     Gets Chassis Manager FRU areas information

wcscli -getpdbassetinfo                Gets PDB FRU areas information

wcscli -setchassismanagerassetinfo     Writes to Chassis Manager FRU Multi Record Area
...
wcscli -clear                          Clear user comamnd history, also clears the client window.
...
Usage: wcscli -getbladehealth [-i <blade_index>] [-q] [-m] [-d] [-p] [-s] [-t] [-f] [-h]
        blade_index - the target blade number. Typically 1-24
...
Usage: wcscli -updatepsufw [-i <psuId>] [-f <filePath>] [-p <isPrimaryImage>]  [-h]
...
Usage: wcscli -setbladeactivepowercycle [-i <blade_index>] | [-a] [-t <off_time>] [-h]
        blade_index - the target blade number. Typically 1-24
        -a - all connected blades
        -t - the time interval in seconds for how long to wait before powering the blade back on;
...
Usage: wcscli -setnextboot [-i <blade_index>]  [-t] <boot_type> [-m] <mode>  [-p] <is_persistent> [-n] <boot_instance> [-h]
        blade_index - the target blade number. Typically 1-24
        -t - boot_type - 1. NoOverRide, 2. ForcePxe, 3. ForceDefaultHdd, 4. ForceIntoBiosSetup, 5. ForceFloppyOrRemovable
        -m - 0. legacy, 1. uefi
        -p - is this a persistent setting (set value 1) or one-time setting (set value 0)
        -n - instance number of the boot device. (Eg. 0 or 1 for NIC if there are two NICs)
        ";
...
Usage: wcscli -setbladepsualertdpc [[-i <blade_index> [-c <default_powercap>] [-t <wait_time>]] | 
                                   [[-a]  [-c <default_powercap>] [-t <wait_time>]] [-h]
        blade_index - the target blade number. Typically 1-24
        -a - all connected blades
        -c - psu alert default power cap
        -t - the time interval in milliseconds for how long to wait before deasserting PROCHOT
        -h - help; display the correct syntax
        ";
...
Usage: wcscli -activatedeactivatebladepsualert [[-i <blade_index> [-c <enable_prochot>] [-d <action] [-r <remove_cap>]] | 
                                               [[-a] [-c <enable_prochot>] [-d <action] [-r <remove_cap>]] [-h] 
        blade_index - the target blade number. Typically 1-24
...
Usage: wcscli –getnic [-h]
        -h – help; display the correct syntax 
        Note that due to WMI caching, if new values are not configured for the subnet mask, gateway IP,
        or DNS servers, the old values will be shown. 
        ";

        public const string setnicHelp = @"
--- (Only available in WCSCLI Serial Mode) --- 
Usage: wcscli -setnic [-a] <IP addr source DHCP/STATIC -Required!> 
                      [-i] <IP address (Required for Static IP)> [-m] <subnetmask (Required for Static IP)>
        ...
        -g - gateway of the chassis controller (Optional for Static IP. Not used for DHCP.)
             Note that the gateway IP can be cleared by switching to DHCP and then back to static IP.
...
//        public const string runwincmdHelp = @"
//--- (Only available in WCSCLI Serial Mode) --- 
//Usage: wcscli –runwincmd [-h]
//        -h – help; display the correct syntax 
//        ";
        #endregion
...
Usage: wcscli -startBladeSerialSession [-i <blade_index>] [-s <session_timeout_in_secs>] [-h]
        blade_index - the target blade number. Typically 1-24
...
Usage: wcscli -startPortSerialSession [-i <Port_index>][-s <session_timeout_in_secs>] [-d <device_timeout_in_millisecs>] 
...
Usage: wcscli -stopbladeserialsession [-h]
        Force termination of any active serial session to any blade.
        -h - help; display the correct syntax
        ";
...
Usage: wcscli -establishCmConnection -m <host_name> -p <port> -s <SSL_option> [-u] <username> [-x] <password> [-b] <batchfileName>
...
Usage: wcscli.exe -h <host_name> -p <port> -s <SSL_option> [-u] <username> [-x] <password> [-b] <batchfileName>
