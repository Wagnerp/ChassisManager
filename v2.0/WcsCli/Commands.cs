...
namespace Microsoft.GFS.WCS.WcsCli
{
    ...
    internal class command
    {
        ...
        internal static uint maxArgCount = 32; // hardcoded maximum number of arguments - argument indicator and the value are counted as one
    ...
    internal class getinfo : command
    {
        ...
        internal override void commandImplementation()
        {
            ...
            if (noArgSpecified == true || this.argVal.ContainsKey('t'))
            {
                ...
                Console.WriteLine();
                Console.WriteLine(WcsCliConstants.getinfoBatteriesHeader);
                myStrings.RemoveAll(item => (1 == 1));
    ...
    /// <summary>
    /// Set Blade Default Power On State class
    /// </summary>
    ...
    /// <summary>
    /// Get Blade Default Power On State class
    /// </summary>
    ...
    internal class powercycle : command
    {
        ...
        internal abstract class SerialSession : command
        {
            ...
            protected void TerminateSession()
            {
                ...
                if (!this.isSerialClient)
                {
                    ...
                    Console.Clear();
                    //Program.autoComplete.ClearHistory();
                    Program.consoleObj.ClearConsoleHistory();
                }
                else
                {
        ...
        internal class startBladeSerialSession : SerialSession, IDisposable
        {
            ...
            // It's value is set to 0 here as this will get automatically translated to the default blade serial session
            ...
            private void EstablishBladeSerialSessionOverSerialClient()
            {
                ...
                while (SharedFunc.ActiveSerialSession)
                {
                    ...
                    for (int i = 0; i < userEnteredData.Length; i++)
                    {
                        ...
                        // which is 0x3 in VT100 encoding
            ...
            internal override void commandImplementation()
            {
                ...
                else
                {
                    ...
                    foreach (NetworkInterfaces.NetworkProperty res in myResponse.NetworkPropertyCollection)
                    {
                        ...
                        {
                            Console.WriteLine(WcsCliConstants.commandSuccess);
                            Console.WriteLine("N/W Interface {0}:", networkInterface);
        ...
        ///// <summary>
        ///// Run Windows command class to execute/launch 
        ///// Windows command prompt in Serial Console session mode
        ///// </summary>
        //internal class RunWinCommand : command
        //{
        //    Process winCmdProcess = new Process();

        //    internal RunWinCommand()
        //    {
        //        this.name = WcsCliConstants.runWinCommand;
        //        this.argSpec.Add('h', null);
        //        this.helpString = WcsCliConstants.runwincmdHelp;
        //    }
        ...
        //        try 
        //        {
        //            // Initialize the process to the Windows command prompt
        //            winCmdProcess.StartInfo.FileName = "cmd.exe";

        //            // Set UseShellExecute to false for redirection.
        //            winCmdProcess.StartInfo.UseShellExecute = false;

        //            // Redirect the standard output of the command prompt.
        //            // This stream is read asynchronously using an event handler.
        //            winCmdProcess.StartInfo.RedirectStandardOutput = true;

        //            // Set our event handler to asynchronously read the command prompt output.
        //            winCmdProcess.OutputDataReceived += new DataReceivedEventHandler(CmdPromptOutputHandler);

        //            // Redirect standard input so that we can use serial stream
        //            winCmdProcess.StartInfo.RedirectStandardInput = true;

        //            // Start the process.
        //            winCmdProcess.Start();

        //            // Start the asynchronous read of the command prompt output.
        //            winCmdProcess.BeginOutputReadLine();

        //            // Connect stream to standard input of the process
        //            System.IO.StreamWriter myStreamWriter = winCmdProcess.StandardInput;

        //            // Read continually until process exits
        //            string inputString;
        //            while (!winCmdProcess.HasExited)
        //            {
        //                // Get user input and send it to the command prompt
        //                inputString = CliSerialPort.ReadUserInputStringFromSerial(new char[] { '\n', '\r' });

        //                if (inputString.ToLower().Contains("exit"))
        //                {
        //                    // End the input stream to the command prompt and close the command prompt process.
        ...
        //                else
        //                {
        //                    // Send user input to command prompt
        //                    myStreamWriter.WriteLine(inputString);
        //                }
        //            }

        //            // Display that command prompt session has ended
        //            CliSerialPort.WriteBytestoSerial(Encoding.ASCII.GetBytes("Command Prompt session has ended" + Environment.NewLine));
        //        }
        //        catch (Exception ex)
        //        {
        //            SharedFunc.ExceptionOutput(ex);
        ...
        //    /// <summary>
        //    /// Event Handler for printing output from command prompt.
        //    /// </summary>
        //    /// <param name="sendingProcess">The sending process.</param>
        //    /// <param name="cmdPromptData">The <see cref="DataReceivedEventArgs"/> instance containing the event data.</param>
        //    private void CmdPromptOutputHandler(object sendingProcess, 
        //        DataReceivedEventArgs cmdPromptData)
        //    {
        //        // Print output from command prompt
        //        if (!String.IsNullOrEmpty(cmdPromptData.Data))
        //        {
        //            CliSerialPort.WriteBytestoSerial(Encoding.ASCII.GetBytes(Environment.NewLine + cmdPromptData.Data));
        //        }
        //    }
        ...
        internal class GetBladeAssetInfo : command
        {
            ...
            internal override void commandImplementation()
            {
                ...
                try
                {
                    if (!(this.argVal.ContainsKey('i')))
                    {
                        Console.WriteLine(WcsCliConstants.commandFailure + " No blade ID specified, please look command help");
                        return;
                    }
                    else
                    {
        ...
        internal class SetChassisManagerAssetInfo : command
        {
            ...
            internal override void commandImplementation()
            {
                ...
                try
                {
                    ...
                    {
                        Console.WriteLine(WcsCliConstants.commandFailure + " No record data specified, please look command help");
                        return;
                    }
                    else
                    {
            ...
            internal override void commandImplementation()
            {
                ...
                if (this.argVal.ContainsKey('a'))
                {
                    foreach (var response in myResponses.bladePsuAlertCollection)
                    {
                        ...
                        {
                            Console.WriteLine(WcsCliConstants.commandSuccess + string.Format(" PSU Alert BMC GPI Status for blade: {0} = {1}", response.bladeNumber, response.PsuAlertGpi));
                ...
                {
                    if (ResponseValidation.ValidateBladeResponse(myResponse.bladeNumber, "Got BIOS POST Code", myResponse, false))
                    {
                        Console.WriteLine(WcsCliConstants.commandSuccess + string.Format(" PSU Alert BMC GPI Status for blade: {0} = {1}", myResponse.bladeNumber, myResponse.PsuAlertGpi));
                        Console.WriteLine("Auto PROCHOT on switch GPI Enabled/Disabled for blade: {0} = {1}",
        ...
        internal class GetBladeBiosPostCode : command
        {
            ...
            internal override void commandImplementation()
            {
                ...
                if (ResponseValidation.ValidateBladeResponse(myResponse.bladeNumber, "Got BIOS POST Code", myResponse, false))
                {
                    Console.WriteLine(WcsCliConstants.commandSuccess + " BIOS POST code for the previous boot is:\n" + myResponse.PreviousPostCode);
                    Console.WriteLine();
                    Console.WriteLine(WcsCliConstants.commandSuccess + " BIOS POST code for the current boot is:\n" + myResponse.PostCode);
