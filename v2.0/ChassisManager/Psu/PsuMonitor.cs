...
namespace Microsoft.GFS.WCS.ChassisManager
{
    ...
    {
        /// <summary>
        /// This variable is required for calculating HoldTime for battery, 
        /// default value 65 secs from emerson spec.
        /// </summary>
        ...
        /// <summary>
        /// PSU Polling and Remediatation of PSU Failures.
        /// </summary>
                        ...
                        else if (psu.Value == PsuAlertFaultType.OnBattery && ConfigLoaded.NumBatteries > 0)
                        {
                            ...
                            if (ConfigLoaded.ProcessBatteryStatus)
                            {
                                    ...
                                    || avgChargeLevel < ConfigLoaded.BatteryChargeLevelThreshold)
                                {
                                    ...
                                    ThreadPool.QueueUserWorkItem(new WaitCallback(NvdimmSupport.ProcessCriticalBatteryStatus));
                                }

                                // Calculate hold time.
                                ushort holdTime = (ushort)(BATT_OP_TIME_100_LOAD * avgChargeLevel / 100);
                                ...
                                Dictionary<byte, CompletionCode> results = WcsBladeFacade.BroadcastSetEnergyStorage
                                    (GetBatteryStateToBroadcast(batteryStates), isBatteryPresent, (byte)avgChargeLevel, holdTime);
