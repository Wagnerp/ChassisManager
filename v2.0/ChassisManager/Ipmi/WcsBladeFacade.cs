...
namespace Microsoft.GFS.WCS.ChassisManager
{
    ...
    public static class WcsBladeFacade
    {
        ...
        /// </summary>
        public static SerialMuxSwitch SetSerialMuxSwitch(byte deviceId)
        {
            if (clients.ContainsKey(deviceId))
            {
                return clients[deviceId].SetSerialMuxSwitch();
            }
            else
            {
        ...
        /// <param name="presence">Energy Stroage Presence</param>
        /// <param name="charge">Percentage Charge remaining</param>
        /// <param name="holdTime">Hold time in seconds</param>
        /// <returns>Collection of Completion Codes</returns>
        internal static Dictionary<byte, CompletionCode> BroadcastSetEnergyStorage(EnergyStorageState state, bool presence, byte charge, ushort holdTime)
        {
            SetEnergyStorageRequest energy = new SetEnergyStorageRequest(state, presence, charge, holdTime, DateTime.Now);

            return BroadcastIpmiRequest<SetEnergyStorageRequest>(energy);
        }
        ...
        /// <summary>
        /// Set the Energy Stroage Status.
        /// </summary>
        /// <param name="presence">energy stroage present</param>
        /// <param name="state">status of energy storage</param>
        /// <param name="precentCharge">percentage charged</param>
        /// <param name="holdTime">hold time in seconds</param>
        /// <param name="timestamp">time stamp</param>
        public static bool SetEnergyStorage(byte deviceId, bool presence, EnergyStorageState state, byte precentCharge, ushort holdTime, DateTime timestamp)
        {
            if (clients.ContainsKey(deviceId))
            {
                return clients[deviceId].SetEnergyStorage(presence, state, precentCharge, holdTime, timestamp);
            }
            else
            {
                return false;
        ...
        /// </summary>
        /// <param name="deviceId">Target blade Id</param>
        /// <param name="flushBuffer">Flush the current BMC data buffer</param>
        /// <param name="Timeout">Session Timeout, zero implies no console session timeout</param>
        /// <returns>StartSerialSession response object</returns>
        public static StartSerialSession StartSerialSession(byte deviceId, bool flushBuffer, int Timeout = 0)
        {
            if (clients.ContainsKey(deviceId))
            {
                return clients[deviceId].StartSerialSession(flushBuffer, Timeout);
            }
            else
            {
        ...
        /// </summary>
        public static StopSerialSession StopSerialSession(byte deviceId)
        {
            if (clients.ContainsKey(deviceId))
            {
                return clients[deviceId].StopSerialSession();
            }
            else
            {
        ...
        /// Send Serial Data
        /// </summary>
        public static SendSerialData SendSerialData(byte deviceId, ushort length, byte[] payload)
        {
            if (clients.ContainsKey(deviceId))
            {
                return clients[deviceId].SendSerialData(length, payload);
            }
            else
            {
        ...
        /// Receive Serial Data
        /// </summary>
        public static ReceiveSerialData ReceiveSerialData(byte deviceId, ushort lenght = ushort.MaxValue)
        {
            if (clients.ContainsKey(deviceId))
            {
                return clients[deviceId].ReceiveSerialData(lenght);
            }
            else
            {
