...
namespace Microsoft.GFS.WCS.ChassisManager
{
    ...
    internal sealed class WcsBladeClient : IpmiClientNodeManager
    {
                    ...
                    else
                    {
                        ...
                        if (this.SensorDataRecords.TryGetValue(sdrKey, out sdr))
                        {
                            ...
                            if (SensorNumber == ConfigLoaded.InletSensor && ConfigLoaded.EnableInletOffSet)
                            {
                                ...
                                {
                                    WcsBladeIdentity id = new WcsBladeIdentity(deviceId.ManufactureId, deviceId.ProductId);
        ...
        /// <returns></returns>
        public SerialMuxSwitch SetSerialMuxSwitch()
        {
            try
            {
                ...
                if (auth.CompletionCode == (byte)CompletionCode.Success)
                {
                    ...
                    if (auth.AuxiliaryData == 0x04)
                    {
                        ...
                        return base.SetSerialMuxSwitch(auth.ChannelNumber);
                    }
                    else
                    {
                        ...
                        // device.
                        SerialMuxSwitch muxswitch = base.SetSerialMux(auth.ChannelNumber, MuxSwtich.BlockRequeststoSystem, false);
            ...
            {
                Tracer.WriteError(String.Format("BladeId: {0} Method: {1} Exception: {2}", this.DeviceId, "SetSerialMuxSwitch", ex.ToString()));
        ...
        /// </summary>
        public SerialMuxSwitch ResetSerialMux()
        {
            try
            {
                ...
                {
                    auth = base.GetAuthenticationCapabilities(PrivilegeLevel.Administrator);
                }
                ...
                if (auth.CompletionCode == (byte)CompletionCode.Success)
                {
                    ...
                    if (auth.AuxiliaryData == 0x04)
                    {
                        return base.ResetSerialMux(auth.ChannelNumber);
                    }
                    else
                    {
                        // JBOD does not require or support SetSerialMuxSwitch. Just return success to complete the sequence of operations
                        return new SerialMuxSwitch((byte)CompletionCode.Success);
                    }
                }
                else
            ...
            {
                Tracer.WriteError(String.Format("BladeId: {0} Method: {1} Exception: {2}", this.DeviceId, "ResetSerialMux", ex.ToString()));
                return new SerialMuxSwitch((byte)CompletionCode.IpmiResponseNotProvided);
            }
        }
        ...
        #region Ipmi Braodcast

        /// <summary>
        /// Sends an Ipmi Request to ever balde in the chassis
        /// </summary>
        ...
        public override StartSerialSession StartSerialSession(bool flushBuffer, int Timeout)
        {
            try
            {
                return base.StartSerialSession(flushBuffer, Timeout);
            }
            catch (Exception ex)
            {
        ...
        /// Receive Serial Data
        /// </summary>
        public override ReceiveSerialData ReceiveSerialData(ushort lenght)
        {
            try
            {
                return base.ReceiveSerialData(lenght);
            }
            catch (Exception ex)
            {
