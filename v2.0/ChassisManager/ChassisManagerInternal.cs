...
namespace Microsoft.GFS.WCS.ChassisManager
{
    ...
    public static class ChassisManagerInternal
    {
        ...
        private static void GetAllBladePwmRequirements()
        {
            ...
            for (byte blade = 1; blade <= MaxSledCount; blade++)
            {
                    ...
                    ChassisState.GetBladeState(blade) == (byte)BladeState.Healthy)
                {
                    ...
                    {
                        Tracer.WriteInfo("GetAllBladePwmRequirements: #### Sledid= " + blade + " Sensor id= " + ConfigLoaded.InputSensor + " Sensor reading= " +
                        ...
                        else
                        {
                            ...
                            {
                                Tracer.WriteWarning("PWM value {0} is out of range (lowThreshold {1} - highThreshold {2})",
                                    Temps.Reading, ConfigLoaded.MinPWM, ConfigLoaded.MaxPWM);
        ...
        #region Chassis FanSpeed, Watchdog, LED & Psu Control Thread Method
