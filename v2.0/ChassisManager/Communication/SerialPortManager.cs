...
namespace Microsoft.GFS.WCS.ChassisManager
{
    ...
    class SerialPortManager : PortManager
    {
        ...
        private const byte setSerialMuxGetMuxSetting = 0x0;
        private const byte setSerialMuxSwitchBmc = 0x2;
        private const byte setSerialMuxSwitchBlockRequeststoSystem = 0x5;

        // Hack to kill serial console on GET/SET NVDIMM Trigger
        private const byte oemGroupCode = 0xC0;
        private const byte setNvdimmTrigger = 0x29;
        private const byte getNvDimmTrigger = 0x2A;
        ...
        protected override void SendReceive(ref WorkItem workItem)
        {
            ...
            // Terminate Serial Session forcefully for NVDIMM
            if (deviceType == (byte)DeviceType.Server)
                NvDimmOverride(request);
        ...
        /// <summary>
        /// Kill Safemode for NVDIMM
        /// </summary>
        private void NvDimmOverride(byte[] request)
        {
            // check if the packet is NVDIMM.
            if ((request[2] == oemGroupCode) && (request[6] == setNvdimmTrigger || request[6] == getNvDimmTrigger))
            {
                // Disable safe mode so that NVDIMM commands can go through
        ...
        private void SendReceiveServer(byte deviceType, byte deviceId, ref byte[] request, out byte[] response)
        {
            ...
            if (ipmiSequenceId <= 0xC0)
            {
                ...
                if (request[2] == setSerialMuxFuncCode && request[netFnIndex] == setSerialMuxCommandNo)
                {
                    // MuxSwtich.BlockRequeststoSystem identifies JBOD.
                    // no payload is required to mux the JBOD.
                    if (request[8] == setSerialMuxSwitchBlockRequeststoSystem)
                    {
                        // we will return a false payload.
                        request = new byte[0];
                        // Enable safe mode for JBOD here since we are not sending any payload on serial port
                        CommunicationDevice.EnableSafeMode();
                    }
                    else if ((request[netFnIndex + 2] == setSerialMuxSwitchBmc) || (request[netFnIndex + 2] == setSerialMuxGetMuxSetting))
                    {
                        Tracer.WriteInfo("SerialPortManager.SendReceiveServer: SetSerialMux to BMC or GetMuxSetting detected");
                    }
                    else
                    {
                        // enable safe mode when entering serial console mode, if completion code is zero.
                        enableSafeMode = true;
                        Tracer.WriteInfo("SerialPortManager.SendReceiveServer: SetSerialMux detected, implicitly assuming safe mode");
                    }
                }
                ...
                if (request.Length > 0)
                {
                    ...
                    // Step 8: Enable Safe mode if SetSerialMux was successful
                    if (dataPacketReceived != null && enableSafeMode)
                    {
                        if (dataPacketReceived.Length >= 7)
                        {
                            // Ipmi Completion Code Success
                            if (dataPacketReceived[7] == 0x00)
                                CommunicationDevice.EnableSafeMode();
                        }
                    }

                    // Step 9: Generate the response packet
                    completionCode = CompletionCode.Success;
                    ResponsePacketUtil.GenerateResponsePacket(completionCode, ref dataPacketReceived, out response);
                }
                else
        ...
        private void SendServerBroadcast(byte deviceType, ref byte[] request, out byte[] response)
        {
            ...
            for (byte bladeId = 1; bladeId <= ConfigLoaded.Population; bladeId++)
            {
                ...
                if (CompletionCodeChecker.Failed(completionCode) == true)
                {
                    Tracer.WriteError("DoSerialTxToSelectServer failed in boardcast. Device Id: {0} Completion Code: {1}",
                ...
                else
                {
                    ...
                    if (CompletionCodeChecker.Failed(completionCode) == true)
                    {
                        Tracer.WriteError("TryToWrite failed in boardcast. Device Id: {0} Completion Code: {1}",
                        deviceId, (byte)completionCode);
                    ...
                    {
                        Tracer.WriteError("Ipmi broadcast IO Exception flushing the serial buffer: {0}", ex);
                    }
        ...
        private void SendReceiveSendConsole(byte deviceType, byte deviceId, ref byte[] request, out byte[] response)
        {
            ...
            if (request.Length > 3)
            {
                if (deviceType == (byte)DeviceType.BladeConsole)
                {
                    ...
                    {
                        byte[] payload = new byte[1] {request[i]};
                        completionCode = TryToWrite(ref payload);
                       if (CompletionCodeChecker.Failed(completionCode) == true)
                       {
                           Tracer.WriteError("SendReceiveSendConsole failed for physical blade: {0} (completionCode: 0x{1:X})",deviceId, completionCode);
                           break;
                       }
                       // wait for byte to write.
                       Thread.Sleep(ConfigLoaded.SerialConsoleDelay);
                    }
                }
                else
                {
