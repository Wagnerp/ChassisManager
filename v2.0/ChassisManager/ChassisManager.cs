...
namespace Microsoft.GFS.WCS.ChassisManager
{
    ...
    public class ChassisManager : IChassisManager
    {
        ...
        public GetAllBladesInfoResponse GetAllBladesInfo()
        {
            ...
            Tracer.WriteInfo("GetAllBladesInfo : Processing Bladees ");
        ...
        public BladeResponse SetPowerOff(int bladeId)
        {
            ...
            BladeSerialSessionMetadata.CompareAndKill(bladeId);
        ...
        public DatasafeBladeResponse SetBladeDatasafeOff(int bladeId)
        {
            ...
            if (!ConfigLoaded.DatasafeAPIsEnabled)
            {
            ...
            DatasafeBladeStatus datasafeResponse = NvdimmSupport.ProcessDatasafeAction(bladeId, DatasafeActions.BladeOff);
        ...
        public DatasafeBladeResponse SetBladeDatasafeOn(int bladeId)
        {
            ...
            // Call ProcessDatasafeAction() so that the current action (command) can be added to the set of pending
            // actions and the processing loop for the pending actions appropriately kicked-off
            DatasafeBladeStatus datasafeResponse = NvdimmSupport.ProcessDatasafeAction(bladeId, DatasafeActions.BladeOn);
            response.RemainingDataSafeDurationInSecs = datasafeResponse.remainingBackupDuration;
            if (datasafeResponse.status == DatasafeCommandsReturnStatus.CommandExecuted)
            {
                response.completionCode = Contracts.CompletionCode.Success;
        ...
        public DatasafeBladeResponse SetBladeDatasafePowerOff(int bladeId)
        {
            ...
            // Call ProcessDatasafeAction() so that the current action (command) can be added to the set of pending
            // actions and the processing loop for the pending actions appropriately kicked-off
            DatasafeBladeStatus datasafeResponse = NvdimmSupport.ProcessDatasafeAction(bladeId, DatasafeActions.PowerOff);
            response.RemainingDataSafeDurationInSecs = datasafeResponse.remainingBackupDuration;
            if (datasafeResponse.status == DatasafeCommandsReturnStatus.CommandExecuted)
            {
                response.completionCode = Contracts.CompletionCode.Success;
        ...
        public DatasafeBladeResponse SetBladeDatasafePowerOn(int bladeId)
        {
            ...
            if (BladeSerialSessionMetadata.ApiGreenSignalling() != CompletionCode.Success)
            {
                Tracer.WriteError("SetBladeDatasafePowerOn({0}) API: Failed to get green signal", bladeId);
            ...
            // Call ProcessDatasafeAction() so that the current action (command) can be added to the set of pending
            // actions and the processing loop for the pending actions appropriately kicked-off
            DatasafeBladeStatus datasafeResponse = NvdimmSupport.ProcessDatasafeAction(bladeId, DatasafeActions.PowerOn);
            response.RemainingDataSafeDurationInSecs = datasafeResponse.remainingBackupDuration;
            if (datasafeResponse.status == DatasafeCommandsReturnStatus.CommandExecuted)
            {
                response.completionCode = Contracts.CompletionCode.Success;
        ...
        public DatasafeBladeResponse SetBladeDatasafeActivePowerCycle(int bladeId)
        {
            ...
            // Call ProcessDatasafeAction() so that the current action (command) can be added to the set of pending
            // actions and the processing loop for the pending actions appropriately kicked-off
            DatasafeBladeStatus datasafeResponse = NvdimmSupport.ProcessDatasafeAction(bladeId, action);
            response.RemainingDataSafeDurationInSecs = datasafeResponse.remainingBackupDuration;
            if (datasafeResponse.status == DatasafeCommandsReturnStatus.CommandExecuted)
            {
                response.completionCode = Contracts.CompletionCode.Success;
        ...
        public DatasafeBladePowerStateResponse GetBladeDatasafePowerState(int bladeId)
        {
            ...
            DatasafeBladeStatus datasafeResponse = NvdimmSupport.GetDatasafeBladeStatus((byte)bladeId);
        ...
        public BladeStateResponse GetBladeDefaultPowerState(int bladeId)
        {
            ...
            if (powerPolicy.CompletionCode != (byte)CompletionCode.Success)
            {
                Tracer.WriteWarning("Get default power state failed with completion code:"
        ...
        public ChassisLogResponse ReadChassisLogWithTimestamp(DateTime start, DateTime end)
        {
            ...
            // Check that end is equal or later than start
            ...
            try
            {
                response.logEntries = UserLogXmllinqHelper.GetFilteredLogEntries(start, endTimeWhenTimeofDayNotPresented, ConfigLoaded.UserLogMaxEntries);
                if (response.logEntries != null)
                {
                    response.completionCode = Contracts.CompletionCode.Success;
                }
        ...
        public BiosPostCode GetPostCode(int bladeId)
        {
            ...
            else
            {
                ...
                // get the previous post code
                response = WcsBladeFacade.GetBiosCode((byte)bladeId, 0x01);

                // provide previous post code if available.  Note: After Blade_EN Off it is not
                // expected for this post code to be availalbe, therefore it is not considered
        ...
        private MultiRecordResponse WriteMultiRecordFru(int deviceId, string recordData, DeviceType deviceType)
        {
            ...
            try
            {
                if (completionCode == (byte)CompletionCode.Success)
                {
                    ...
                    int recordLength = (multiRecordInfo.HeaderSize + record.Length + manufacturerIdLen + 1);
                    ...
                    // This totals as 125 bytes. So if maxBytes < 126 the entire payload size follows the limit. 
