...
namespace ChassisValidation
{
    ...
    internal class BladeCommands : CommandBase
    {
        ...
        public bool SetGetAllBladesStateTest()
        {
            ...
            // test blade state on
            CmTestLog.Info("Trying to set all blade states to on");
        ...
        public bool SetGetBladeDefaultPowerStateTest()
        {
            ...
            {
                int radEmptySlot = JbodLocations.RandomOrDefault();
        ...
        public bool SetAllBladesActivePowerCycleByAllUsers()
        {
            ...
            foreach (WCSSecurityRole roleId in Enum.GetValues(typeof(WCSSecurityRole)))
            {
                try
                {
                    ...
                    allPowerResponse = this.TestChannelContext.SetAllBladesActivePowerCycle(Convert.ToUInt32(offTime.TotalSeconds));
        ...
        public bool SetBladeActivePowerCycleByAllUsers()
        {
            ...
            foreach (WCSSecurityRole roleId in Enum.GetValues(typeof(WCSSecurityRole)))
            {
                try
                {
                    ...
                    bladeRes = this.TestChannelContext.SetBladeActivePowerCycle(bladeId, Convert.ToUInt32(offTime.TotalSeconds));
        ...
        public bool SetAllBladesActivePowerCycleTest()
        {
            ...
            var response = this.Channel.SetAllBladesActivePowerCycle(Convert.ToUInt32(offTime.TotalSeconds));
            ...
            response = this.Channel.SetAllBladesActivePowerCycle(Convert.ToUInt32(offTime.TotalSeconds));
            ...
            CmTestLog.Info("Trying to set blade active power cycle for all blades with -ve offtime");
            try
            {
                response = this.Channel.SetAllBladesActivePowerCycle(Convert.ToUInt32(offTime.TotalSeconds));
            }
            catch (Exception ex)
            {
                ...
                CmTestLog.Failure("Request error for -ve offtime.\n");
            }

            // Verify command fails and blade state is not affected
            Thread.Sleep(2000);
            CmTestLog.Info("Verifying all blades state not affected after set -ve off time");
            bladeStates = this.Channel.GetAllBladesState();
            allPassed &= this.VerifyBladeState(PowerState.ON, bladeStates.bladeStateResponseCollection);
        ...
        private bool VerifyJbodBladeInfo(bool testPassed, BladeInfoResponse bladeInfo)
        {
            ...
            if (bladeInfo.macAddress.Count != 0)
            {
                foreach (var nicInfo in bladeInfo.macAddress)
                {
                    testPassed &= ChassisManagerTestHelper.AreEqual(nicInfo.completionCode, CompletionCode.Failure, "Received Failure completion code for NicInfo");
        ...
        private void VerifyServerFruInformation(ref bool allPassed, BladeHealthResponse bladeHealth, int bladeId, XmlReader bladehealthInfo)
        {
            ...
            allPassed &= ChassisManagerTestHelper.IsTrue(bladeHealth.serialNumber == serialNumber, string.Format("Received SerialNumber for blade# {0}", bladeHealth.bladeNumber));
        ...
        private void VerifyPCIeInformation(ref bool allPassed, BladeHealthResponse bladeHealth, XmlReader bladehealthInfo)
        {
            ...
            foreach (PCIeInfo serverPCIeInfo in bladeHealth.pcieInfo)
            {
                ...
                allPassed &= ChassisManagerTestHelper.AreEqual(propertyValue, serverPCIeInfo.vendorId,
                    string.Format("Recieved PCIe VendorId for blade:PCIeNumber {0}:{1}", bladeHealth.bladeNumber, serverPCIeInfo.pcieNumber));
                ...
                allPassed &= ChassisManagerTestHelper.AreEqual(propertyValue, serverPCIeInfo.deviceId,
                    string.Format("Recieved PCIe DeviceId for blade:PCIeNumber {0}:{1}", bladeHealth.bladeNumber, serverPCIeInfo.pcieNumber));
                ...
                allPassed &= ChassisManagerTestHelper.AreEqual(propertyValue, serverPCIeInfo.subSystemId,
                    string.Format("Reecived SubsystemId for blade:PCIeNumber {0}:{1}", bladeHealth.bladeNumber, serverPCIeInfo.pcieNumber));
        ...
        private void VerifyMemoryInformation(ref bool allPassed, BladeHealthResponse bladeHealth, XmlReader bladehealthInfo)
        {
            ...
            foreach (MemoryInfo serverDimmInfo in bladeHealth.memoryInfo)
            {
                ...
                propertyValue = bladehealthInfo.ReadElementContentAsString();
                allPassed &= ChassisManagerTestHelper.AreEqual(ushort.Parse(propertyValue), serverDimmInfo.speed, string.Format("Received DIMM speed for blade:DIMM {0}:{1}", bladeHealth.bladeNumber, serverDimmInfo.dimm));

                bladehealthInfo.ReadToFollowing("size");
                propertyValue = bladehealthInfo.ReadElementContentAsString();
                allPassed &= ChassisManagerTestHelper.AreEqual(ushort.Parse(propertyValue), serverDimmInfo.size, string.Format("Received DIMM size for blade:DIMM {0}:{1}", bladeHealth.bladeNumber, serverDimmInfo.dimm));
        ...
        private void VerifyProcessorInformation(ref bool allPassed, BladeHealthResponse bladeHealth, XmlReader bladehealthInfo)
        {
            ...
            foreach (ProcessorInfo serverProcInfo in bladeHealth.processorInfo)
            {
                ...
                propertyValue = bladehealthInfo.ReadElementContentAsString();
                allPassed &= ChassisManagerTestHelper.AreEqual(ushort.Parse(propertyValue), serverProcInfo.frequency,
