...
namespace Microsoft.GFS.WCS.ChassisManager.Ipmi
{
    ...
    internal abstract class IpmiClientExtended : IpmiClientAdvance
    {
        ...
        public virtual PCIeInfo GetPCIeInfo(byte device)
        {
            ...
            {
                if (pcieInfo.VendorId == 65535 && pcieInfo.SubsystemVendorId == 65535)
                {
        ...
        public virtual EnergyStorage GetEnergyStorage()
        {
            ...
            EnergyStorage response = new EnergyStorage(energyStorage.CompletionCode);
             ...
             {
                 response.SetParamaters(energyStorage.Precence, energyStorage.EnergyState,
                     energyStorage.PercentageCharge, energyStorage.HoldTime, energyStorage.TimeStamp);
             }
        ...
        /// </summary>
        /// <param name="presence">energy storage present</param>
        ...
        /// <returns></returns>
        public virtual bool SetEnergyStorage(bool presence, EnergyStorageState state, byte precentCharge, ushort holdTime, DateTime timestamp)
        {
            SetEnergyStorageResponse response = (SetEnergyStorageResponse)this.IpmiSendReceive(
                new SetEnergyStorageRequest(state, presence, precentCharge, holdTime, timestamp), typeof(SetEnergyStorageResponse));
        ...
        public virtual StartSerialSession StartSerialSession(bool flushBuffer, int Timeout = 0)
        {
            StartSerialSessionResponse ipmiResponse = (StartSerialSessionResponse)this.IpmiSendReceive(
                new StartSerialSessionRequest(flushBuffer, Timeout), typeof(StartSerialSessionResponse));
        ...
        /// Receive Serial Data
        /// </summary>
        public virtual ReceiveSerialData ReceiveSerialData(ushort lenght = ushort.MaxValue)
        {
            ReceiveSerialDataResponse ipmiResponse = (ReceiveSerialDataResponse)this.IpmiSendReceive(
                new ReceiveSerialDataRequest(lenght), typeof(ReceiveSerialDataResponse));

            ReceiveSerialData response = new ReceiveSerialData(ipmiResponse.CompletionCode);

            if (response.CompletionCode == 0x00)
            {
                response.SetParamaters(ipmiResponse.PayloadLenght, ipmiResponse.Payload);
            }
